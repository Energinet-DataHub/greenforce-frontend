...
export class DashboardPo {
  ...
  pieChartIsVisible = () =>
    cy.get('eo-dashboard-chart-card').should('contain.html', 'eo-origin-of-energy-pie-chart');
  exportDataCardIsVisible = () => cy.get('eo-dashboard-get-data').should('be.visible');
  emissionCardIsVisible = () =>
    cy.get('eo-dashboard-emissions-card').should('contain.html', 'eo-emissions-data');
