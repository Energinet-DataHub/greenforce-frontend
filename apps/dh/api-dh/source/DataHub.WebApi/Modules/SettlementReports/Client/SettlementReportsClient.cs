...
public class SettlementReportsClient(
    IHttpContextAccessor httpContextAccessor,
    ISettlementReportClient client,
    ICalculationsClient calculationsClient,
    IMarketParticipantClient_V1 marketParticipantClient,
    IGridAreaByIdDataLoader gridAreaDataLoader) : ISettlementReportsClient
{
    public async Task<RequestedSettlementReportDto> GetSettlementReportByIdAsync(string id, CancellationToken ct) =>
        (await client.GetAsync(ct)).First(r => r.RequestId.Id == id);

    public async Task<IEnumerable<RequestedSettlementReportDto>> GetSettlementReportsAsync(
        CancellationToken ct) => await client.GetAsync(ct);

    public async Task<Dictionary<string, List<SettlementReportApplicableCalculation>>?>
            ...
            Interval calculationPeriod)
    {
        ...
        var currentActor = await marketParticipantClient.ActorGetAsync(currentActorId);
    ...
    {
        if (input.RequestAsMarketRole is null)
        {
            throw new ArgumentException(
                "Invalid market role for settlement report request.",
                nameof(input.RequestAsMarketRole));
        }

        var requestAsActor = Guid.TryParse(input.RequestAsActorId, out var actorNumber)
            ? await marketParticipantClient.ActorGetAsync(actorNumber)
            : null;

        var requestFilter = new SettlementReportRequestFilterDto(
            input.GridAreasWithCalculations.ToDictionary(
                x => x.GridAreaCode,
    ...
    public async Task<bool> CancelSettlementReportAsync(
        ...
        CancellationToken ct)
    {
        var reportRequestId = new ReportRequestId(id);
        await client.CancelAsync(reportRequestId, ct);
        return true;
    }

    private static Guid GetCalculationId(WholesaleCalculationResultV1 calculation)
    {
