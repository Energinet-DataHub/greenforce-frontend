...
public static class ActorDtoExtension
{
    public static MarketParticipantStatus GetStatus(this ActorDto actor)
    {
        if (actor.ConsolidateAt is null)
        {
            return Enum.Parse<MarketParticipantStatus>(actor.Status);
        }

        if (actor.ConsolidateAt < DateTimeOffset.UtcNow)
        {
