...
public class AuthorizedHttpClientFactory
{
    ...
    public ActorConversationClient_V1 CreateActorConversationClient()
    {
        var client = _httpClientFactory.CreateClient();
        SetAuthorizationHeader(client);
        return new ActorConversationClient_V1(client) { BaseUrl = _baseUrls.Value.ActorConversationBaseUrl };
    }
