...
export interface EoOriginOfEnergyState {
  renewable: number;
  other: number;
}

const initialState: EoOriginOfEnergyState = {
  renewable: 0,
  other: 0,
};
...
export class EoOriginOfEnergyStore extends ComponentStore<EoOriginOfEnergyState> {
  share$: Observable<EoOriginOfEnergyState> = this.select(
    this.originOfEnergyService.getEmissions().pipe(
      tapResponse(
        (response) => response,
        (error) => {
  ...
  constructor(private originOfEnergyService: EoOriginOfEnergyService) {
    super(initialState);
  }
}
