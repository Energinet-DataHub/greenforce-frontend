...
import { EttAssignmentAddIconComponent } from './assignment-add.component';

@Component({
  changeDetection: ChangeDetectionStrategy.OnPush,
  encapsulation: ViewEncapsulation.None,
  selector: 'ett-grant-consent-modal',
  imports: [
    WATT_MODAL,
    WattIconComponent,
    ...
    TranslocoPipe,
    WattButtonComponent,
    EttAssignmentAddIconComponent,
    EttGenitivePipe,
    EttConsentPermissionsComponent,
  ],
  styles: `
    .ett-grant-consent-modal .watt-modal {
  ...
  template: `
    @if (opened) {
      <watt-modal
        ...
        [disableClose]="true"
        [loading]="isLoading()"
        [panelClass]="['ett-grant-consent-modal']"
      >
        @if (!hasError()) {
          <watt-icon style="color: #00847C" size="xxl" class="icon">
            <ett-assignment-add-icon />
          </watt-icon>
          <h3>
            {{
          ...
          <ett-consent-permissions [serviceProviderName]="organizationName()" />
...
export class EttGrantConsentModalComponent {
