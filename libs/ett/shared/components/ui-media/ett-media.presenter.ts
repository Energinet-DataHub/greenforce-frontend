...
@Injectable()
export class EttMediaPresenter extends ComponentStore<EttMediaState> {
   ...
   */
  mediaImagettrder$: Observable<number> = this.select(
  ...
  updateMediaMaxWidthPixels = this.updater<number | null>(
    (state, mediaMaxWidthPixels): EttMediaState => ({
      ...state,
      mediaMaxWidthPixels,
    })
  );
   ...
   * Defaults to 'start'.
   */
  updateMediaImageAlign = this.updater<EttMediaAlign | null>(
    (state, mediaImageAlign): EttMediaState => ({
      ...state,
  ...
  updateMediaImageMaxWidthPixels = this.updater<number | null>(
    (state, mediaImageMaxWidthPixels): EttMediaState => ({
      ...state,
...
const defaultMediaImageAlign: EttMediaAlign = 'start';
const initialState: EttMediaState = {
  gapPixels: null,
