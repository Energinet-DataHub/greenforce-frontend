...
import { EttCertificate, EttCertificateType } from '@energinet-datahub/ett/certificates/domain';
import {
  EttCertificatesService,
  sortCertificatesBy,
} from '@energinet-datahub/ett/certificates/data-access-api';
...
interface CertificateFiltersForm {
  ...
  type: FormControl<EttCertificateType[]>;
}
...
export class EttCertificatesOverviewComponent implements OnInit {
  ...
  /* Certificates type */
  private _type?: EttCertificateType;
  @Input()
  set type(value: EttCertificateType | undefined) {
    this._type = value;
  }
  get type() {
    return this._type;
  ...
  @ViewChild(WattPaginatorComponent) paginator!: WattPaginatorComponent<EttCertificate>;
  ...
  protected dataSource!: AsyncDataSource<EttCertificate>;
  protected exportingCertificates = signal<boolean>(false);

  ngOnInit() {
    this.dataSource = new AsyncDataSource<EttCertificate>([], (this.page ?? 1) - 1, this.pageSize);
    this.initForm();
    ...
    this.transloco
      ...
      .subscribe(() => {
        ...
        this.form.valueChanges.pipe(takeUntilDestroyed(this.destroyRef)).subscribe((changes) => {
          ...
          if (changes.period) {
            this.router.navigate([this.transloco.getActiveLang(), ettCertificatesRoutePath], {
              queryParams: {
  ...
  private initForm() {
    ...
    const type: EttCertificateType[] = this.type
      ? [this.type]
      : [EttCertificateType.Production, EttCertificateType.Consumption];
    ...
    this.form = new FormGroup({
      ...
      type: new FormControl<EttCertificateType[]>(type, {
        nonNullable: true,
      }),
    });
  }
  ...
  goToDetails(element: EttCertificate) {
  ...
  private setColumns(): void {
    this.columns = {
      ...
      certificateType: {
        accessor: (x) => {
          if (x.certificateType.toLowerCase() === EttCertificateType.Production) {
            return this.transloco.translate(this.translations.certificates.productionType);
          } else if (x.certificateType.toLowerCase() === EttCertificateType.Consumption) {
  ...
  private loadData() {
    ...
    this.certificatesService
      ...
      .subscribe({
        next: (certificates) => {
          this.dataSource.data = certificates.result as EttCertificate[];
          if (this.dataSource.paginator) {
