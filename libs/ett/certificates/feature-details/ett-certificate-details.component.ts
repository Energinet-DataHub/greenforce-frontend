...
import { AibTechCode } from '@energinet-datahub/ett/metering-points/domain';
...
export class EttCertificateDetailsComponent implements OnInit {
  ...
  certificate = signal<EttCertificate | null>(null);
  ...
  ngOnInit(): void {
    ...
    if (!registry || !streamId) {
      this.router.navigate([`/${ettCertificatesRoutePath}`]);
      return;
    }
