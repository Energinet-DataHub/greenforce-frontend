...
import { EttReceiverInputComponent } from './ett-receiver-input.component';
import { EttSenderInputComponent, Sender } from './ett-sender-input.component';
...
export interface EttTransfersFormValues {
  senderTin?: string;
  receiverTin: string;
...
@Component({
  ...
  imports: [
    ...
    VaterStackComponent,
    TranslocoPipe,
    EttReceiverInputComponent,
    EttSenderInputComponent,
  ],
  ...
  styles: [
    `
      ...
      ett-transfers-form .parties-step,
      ...
      ett-transfers-form .sender .watt-field-wrapper {
        max-width: 330px;
      }
...
export class EttTransfersFormComponent implements OnInit {
  ...
  initialValues = input<EttTransfersFormInitialValues>({
    senderTin: '',
    receiverTin: '',
  ...
  submitted = output<EttTransfersFormValues>();
  canceled = output();

  @ViewChild('invitationLink') invitationLink!: EttTransferInvitationLinkComponent;
  ...
  createTransferAgreementProposal() {
    ...
    const ettTransfersFormValues: EttTransfersFormValues = {
    ...
    this.submitted.emit(ettTransfersFormValues);
  ...
  private addConsentLabel(receiverTin: string): string {
    return `${receiverTin} (${this.transloco.translate(this.translations.consent.consent)})`;
  }
