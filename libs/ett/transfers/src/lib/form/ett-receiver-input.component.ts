...
import { FormMode } from './ett-transfers-form.component';

@Component({
  selector: 'ett-receiver-input',
  imports: [TranslocoPipe, WattFieldErrorComponent, WattFieldHintComponent, WattTextFieldComponent],
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => EttReceiverInputComponent),
      multi: true,
    },
    {
      provide: NG_VALIDATORS,
      multi: true,
      useExisting: EttReceiverInputComponent,
    },
  ],
  styles: [
    `
  ...
  template: `
    <div class="flex-center">
      <watt-text-field
        ...
        (autocompletettptionSelected)="onSelectedReceiver($event)"
        (autocompletettptionDeselected)="selectedCompanyNameChange.emit(undefined)"
        [autocompletettptions]="filteredReceiverTins() || []"
...
export class EttReceiverInputComponent implements ControlValueAccessor, Validator {
