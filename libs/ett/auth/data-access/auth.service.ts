...
export interface EttUser extends User {
  profile: {
    sub: string;
    iss: string;
    aud: string;
...
export class EttAuthService {
  ...
  private b2cEnvironment: EttB2cEnvironment = inject(ettB2cEnvironmentToken);
  private apiEnvironment: EttApiEnvironment = inject(ettApiEnvironmentToken);
  ...
  signinCallback(): Promise<User | null> {
    return this.userManager
      ? this.userManager?.signinCallback().then((user) => {
          if (user) {
            this.user.set((user as EttUser) ?? null);
          }
