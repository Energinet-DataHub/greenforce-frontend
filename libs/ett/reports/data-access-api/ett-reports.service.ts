...
export class EttReportsService implements OnDestroy {
  #apiBase: string;
  ...
  readonly #reports = signal<EttReport[]>([]);
  ...
  startReportGeneration(newReportRequest: EttReportRequest) {
    return this.http.post<EttReportRequest>(`${this.#apiBase}/reports`, {
  ...
  getReports() {
    return this.http.get<EttReportResponse>(`${this.#apiBase}/reports`);
  }

  downloadReport(report: EttReport): void {
    this.http
