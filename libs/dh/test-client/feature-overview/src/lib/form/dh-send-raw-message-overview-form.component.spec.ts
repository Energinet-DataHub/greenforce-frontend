...
//import { dhSendMessagePath } from '@energinet-datahub/dh/metering-point/feature-overview';
import {
  validMeteringPointId,
  invalidMeteringPointId,
} from '@energinet-datahub/dh/shared/test-util-metering-point';

import { DhSendMessageComponent } from '../dh-send-message.component';
import {
  DhMeteringPointSearchFormComponent,
  DhMeteringPointSearchFormScam,
} from './dh-send-message-form.component';
import { dhSendMessagePath } from '../routing/dh-send-message-path';

describe(DhSendMessageFormComponent.name, () => {
  async function setup() {
    const { fixture, navigate } = await render(
      DhMeteringPointSearchFormComponent,
      {
        ...
        routes: [
          {
            path: `${dhSendMessagePath}/${dhMeteringPointSearchPath}`,
            component: DhSendMessageComponent,
          },
        ],
      }
    );
