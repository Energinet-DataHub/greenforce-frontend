...
@Component({
  selector: 'dh-request-incorrect-change-of-supplier-modal',
  ...
  template: `
    <watt-modal
      *transloco="let t; prefix: 'meteringPoint.processOverview.incorrectChangeOfSupplier'"
      [title]="t('title')"
      size="small"
    >
      <form
      ...
      <watt-modal-actions>
        ...
        <watt-button
          type="submit"
          formId="request-incorrect-change-of-supplier-form"
          [loading]="loading()"
        >
          {{ t('confirm') }}
        </watt-button>
...
export class DhRequestIncorrectChangeOfSupplierModal extends WattTypedModal<RequestIncorrectChangeOfSupplierModalData> {
  ...
  readonly loading = this.requestIncorrectChangeOfSupplierMutation.loading;
  ...
  async submit() {
    ...
    await this.requestIncorrectChangeOfSupplierMutation.mutate({
      refetchQueries: [
        GetMeteringPointProcessByIdDocument,
        GetMeteringPointProcessOverviewDocument,
      ],
