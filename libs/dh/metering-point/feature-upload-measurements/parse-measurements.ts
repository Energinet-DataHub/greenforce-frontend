...
const mapQuality = (quality: string): SendMeasurementsQuality | null => {
  switch (quality) {
    case 'A03':
...
const aggregate = (result: MeasureDataResult, step: ParseStep, index: number) => {
  ...
  if (currentEnd?.isBefore(result.last)) return result.fatal('MISSING_MEASUREMENT');
  if (currentEnd?.isAfter(result.last)) return result.fatal('UNEXPECTED_MEASUREMENT');
...
/** Parse measurement data, streaming the result. */
export const parseMeasurements = (
  ...
  return of(source).pipe(
    ...
    scan(aggregate, new MeasureDataResult(size, resolution)),
