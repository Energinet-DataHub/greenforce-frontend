...
import { graphql, UserRoleWithPermissionsDto } from '@energinet-datahub/dh/shared/domain';
import { Permission } from '../../../../../../../shared/domain/src/lib/generated/graphql';

@Component({
  selector: 'dh-permission-detail-audit-logs',
  standalone: true,
  templateUrl: './dh-permission-detail-audit-logs.component.html',
  styles: [
    `
      :host {
        display: block;
...
export class DhPermissionDetailsAuditLogsComponent implements OnInit, OnChanges {
  ...
  reloadAuditLogs(): void {
    if (this.selectedPermission) {
      //this.store.getAuditLogs(this.selectedPermission.id);
    }
  }
}
