...
function getMeteringPointProcessOverview() {
  return mockGetMeteringPointProcessOverviewQuery(async () => {
    ...
    const endOfSupplyProcess = {
      ...
      availableActions: [WorkflowAction.CancelWorkflow, WorkflowAction.ConfirmWorkflow, WorkflowAction.RejectRequest],
      initiator: {
        __typename: 'MarketParticipant' as const,
        ...initiators[0],
      },
...
function getAvailableActions(
  ...
  if (businessReason === ProcessManagerBusinessReason.EndOfSupply)
    return [WorkflowAction.CancelWorkflow, WorkflowAction.ConfirmWorkflow, WorkflowAction.RejectRequest];
