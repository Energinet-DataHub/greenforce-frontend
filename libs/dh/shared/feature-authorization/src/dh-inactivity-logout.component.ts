...
@Component({
  ...
  template: `
    <watt-modal *transloco="let t" [size]="'small'" [title]="t('sessionExpirationTitle')">
      ...
      <h2>{{ warningCountdown() | date: 'mm:ss' }}</h2>
    </watt-modal>
  `,
  imports: [DatePipe, TranslocoDirective, WATT_MODAL],
})
export class DhInactivityLogoutComponent extends WattTypedModal {
  private readonly secondsUntilLogOff = 10;

  readonly warningCountdown = toSignal(
    timer(0, 1000).pipe(
    ...
    { initialValue: new Date((this.secondsUntilLogOff - 1) * 1000) }
  );
}
