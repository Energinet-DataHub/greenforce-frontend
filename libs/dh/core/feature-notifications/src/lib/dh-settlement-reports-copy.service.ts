...
type SettlementReportPartial = Pick<
  DhSettlementReport,
  'period' | 'calculationType' | 'gridAreas' | 'settlementReportDownloadUrl'
>;

@Injectable({
  providedIn: 'root',
})
export class DhSettlementReportsCopyService {
  private toastService = inject(WattToastService);

  private addTokenToDownloadUrlMutation = mutation(AddTokenToDownloadUrlDocument);

  async downloadReport(settlementReport: SettlementReportPartial) {
  ...
  private settlementReportName(report: SettlementReportPartial): string {
