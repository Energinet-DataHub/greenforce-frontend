...
import { DhMeteringPointNewConversationComponent } from './metering-point-new-conversation.component';
import { ActorConversationStore } from '../actor-conversation.store';

@Component({
  selector: 'dh-metering-point-conversation-shell',
  imports: [
    TranslocoDirective,
    VATER,
    WATT_CARD,
    ...
    DhMeteringPointNewConversationComponent,
    VaterStackComponent,
        ...
        <vater-stack scrollable *transloco="let t; prefix: 'meteringPoint.actorConversation'">
          @switch (store.state()) {
            @case ('newConversationOpen') {
              <dh-metering-point-new-conversation
                vater
                fill="both"
...
export class DhMeteringPointConversationShellComponent {
  protected readonly store = inject(ActorConversationStore);

  /** Required — this shell always operates within a metering point context */
  meteringPointId = input.required<string>();

  private readonly syncMeteringPointId = effect(() => {
