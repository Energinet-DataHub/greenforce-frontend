...
export interface EovLoginToken {
  atv?: number;
  exp?: number;
  name?: string;
  nbf?: number;
...
type AuthState = EovLoginToken;

@Injectable({
  providedIn: 'root',
})
export class EovAuthStore extends ComponentStore<AuthState> {
  ...
  setTokenClaims = this.updater(
    (state, claim: EovLoginToken): AuthState => ({ ...state, ...claim })
  );
}
