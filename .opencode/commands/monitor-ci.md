...
**Important:** Within the current agent session, conversation context persists. If you interrupt the monitor and re-run `/monitor-ci`, the agent remembers the previous state and may continue from where it left off.
...
- **For a completely clean slate:** End the current agent session and start a new one (for example, open a new chat or workspace)
