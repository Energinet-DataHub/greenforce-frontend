...
Keep in mind that you might have to prefix commands with `npx`/`pnpx`/`yarn`/`bunx` (or use the workspace's configured package manager) if Nx isn't installed globally. Check the lockfile to determine the package manager in use.
